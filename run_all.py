...
def cluster_vector_file(file_path, cluster_count, output_dir):
    os.chdir(output_dir)
    real_cluto_params = default_cluto_params.format(input_file=file_path, cluster_count=cluster_count, cluto_log= file_name +'.'+ str(cluster_count)+ '_log')
...
if __name__ == "__main__":
    ...
    for cluto_file_name in sorted(os.listdir(cluto_files_dir)):
        base_name = cluto_file_name.split('.')[0]
        print('==={0}==='.format(base_name))
        orig_file = os.path.join(cluto_files_dir, '{0}.original.txt'.format(base_name)) #input_dir
        sparse_file = os.path.join(cluto_files_dir, '{0}.sparse.clustering.2'.format(base_name))
        # Process files
